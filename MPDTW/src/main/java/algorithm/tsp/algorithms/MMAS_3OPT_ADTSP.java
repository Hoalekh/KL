package algorithm.tsp.algorithms;

import algorithm.tsp.aco.Ant;
import algorithm.tsp.aco.MMAS;
import algorithm.tsp.aco.ls.opt3.Opt3Operator;
import algorithm.tsp.graph.Graph;
import algorithm.tsp.graph.Vertex;
import algorithm.tsp.tools.DBGP;
import algorithm.statistic.GlobalStatistics;
 ...
 */
public class MMAS_3OPT_ADTSP implements Runnable {
    ...
    public void executeLocalSearch() {
        ...
        opt3Operator.init(graph, random, iterationBest.getTour().clone());
        opt3Operator.optimize();
        int result[] = opt3Operator.getResult();
        double cost = mmas.fitnessEvaluation(result);
        if (cost < iterationBest.getCost()) {
            iterationBest.setTour(result);
            iterationBest.setCost(cost);
